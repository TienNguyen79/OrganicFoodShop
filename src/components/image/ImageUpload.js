...
const ImageUpload = ({
  ...
  const handleDeleteImage = async () => {
    setShowImage("");
    setValue1 && setValue1(name, "");
  };
  return (
    <div className="flex">
      <label
        className={` w-[200px]  border border-gray-200  border-dashed overflow-hidden  h-[200px]  group cursor-pointer flex items-center justify-center ${className}`}
      >
      ...
      {showImage && (
        <div>
          <button
            type="button"
            className={`w-14 h-14 flex  ml-3  items-center bg-white  ${className} text-red-500`}
          >
